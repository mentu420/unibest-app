...
import { useRect, useEventListener, useWindowSize } from '@vant/use'
// Utils
import { pick, addUnit, closest, makeNumberProp, makeStringProp } from './utils'
...
export default defineComponent({
  ...
  setup(props, { slots, emit, attrs }) {
    ...
    return () => {
      const Content = (
        <div
          class="fixed left-2 top-2 z-[1000]"
          ref={rootRef}
          ...
          {...attrs}
        >
          {slots.default ? slots.default() : <uni-icons type={props.icon} />}
        </div>
      )
      return props.teleport ? <Teleport to={props.teleport}>{Content}</Teleport> : Content
    }
  },
})
