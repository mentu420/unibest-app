  ...
  <view class="zsy_calendar">
        ...
        <text class="desc">
          ({{
            getAssignDateInfo(false, 0) === getAssignDateInfo(true, 0)
              ? ''
              : getAssignDateInfo(false, 0) + '年'
          }}{{ getAssignDateInfo(false, 1) }}月)
        </text>
        <text
          v-show="showBackToTodayBtn"
          class="backToToday"
          :style="{ color: dateActiveColor }"
          @tap="goToDate()"
        >
          回到今天
        </text>
      ...
      >
        <swiper-item v-for="(swiper, swiperIndex) in 3" :key="swiperIndex" class="swiper-item">
          <DateBox
            ...
            :date-active-color="dateActiveColor"
            :swiper-mode="swiperMode"
    ...
    <view class="calendar_toggle" @tap="swiperMode = swiperMode === 'open' ? 'close' : 'open'">
      <view
        ...
        :class="swiperMode === 'close' ? 'i-uil-expand-arrows' : 'i-uil-compress-arrows'"
      ></view>
    </view>
  </view>
</template>
...
import { parseTime, deepClone } from './js/utils'
export default {
  name: 'ZsyCalendar',
  components: {
    DateBox,
  ...
  computed: {
    ...
    swiperHeight() {
      return (swiperMode) => {
        const normalHeight =
          ((this.calendarSwiperDates[this.current] || []).length / 7) * (this.cellHeight + 20) +
          'rpx'
  ...
  methods: {
    ...
    initCalendarShrinkSwiperDates(swiperChangeType) {
      ...
      if (
        ...
        (swiperChangeType === -1 &&
          curDateLine !== 0 &&
          this.calendarSwiperDates[this.current][(curDateLine - 1) * 7].type === 'cur') ||
        (swiperChangeType === 1 &&
    ...
    goToDate(date = this.today) {
      try {
        if (date.split('-').length < 2 || date.split('-').length > 3) throw new Error('参数有误')
    ...
    // 向父组件传递当前选中数据
    emitDate() {
      if (this.calendarSwiperDates.length === 0) return
